...
class AddressController {
    ...
    async getByAddress(req, res) {
        try {
            ...
            try {
                ...
                try {
                    const addrInfo = await this.rpc.call('getaddressinfo', [address]);
                ...
                const scanResult = await this.rpc.call('scantxoutset', ['start', descriptors]);
                if (scanResult && scanResult.success) {
                    ...
                    utxos = deduped;
                    balance = utxos.reduce((s, u) => s + (u.amount || 0), 0);
                ...
                try {
                    const txs = await this.rpc.call('listtransactions', ['*', 1000, 0, true]);
                    if (Array.isArray(txs)) {
                        const addrTxs = txs.filter((tx) => tx.address === address);
    ...
    async getTransactions(req, res) {
        try {
            const { address } = req.params;
            const count = parseInt(req.query.count) || 999999;
            ...
            try {
                ...
                if (scanResult && scanResult.success && scanResult.unspents) {
                    // Deduplicate
                    const seen = new Set();
                    scanResult.unspents = scanResult.unspents.filter((u) => {
                        const key = `${u.txid}:${u.vout}`;
                        return seen.has(key) ? false : (seen.add(key), true);
                    });
                    // Fetch full transaction details for each UTXO
                    const txDetails = await Promise.all(scanResult.unspents.slice(skip, skip + count).map(async (utxo) => {
                        try {
                ...
                try {
                    const txs = await this.rpc.call('listtransactions', ['*', count, skip, true]);
                    result = Array.isArray(txs) ? txs.filter((tx) => tx.address === address) : [];
                }
                catch {
                    result = [];
                }
        ...
        try {
            ...
            try {
                ...
                if (scanResult && scanResult.success) {
                    // Deduplicate UTXOs
                    const seen = new Set();
                    utxos = (scanResult.unspents || []).filter((u) => {
                        const key = `${u.txid}:${u.vout}`;
                        return seen.has(key) ? false : (seen.add(key), true);
                    });
                }
