...
const char * const BITCOIN_CONF_FILENAME = "tarcoin.conf";
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Tarcoin
    //   new: C:\Users\Username\AppData\Local\Tarcoin
    // macOS: ~/Library/Application Support/Tarcoin
    // Unix-like: ~/.tarcoin
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Tarcoin";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Tarcoin";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/Tarcoin";
#else
    // Unix-like
    return pathRet / ".tarcoin";
#endif
#endif
}
