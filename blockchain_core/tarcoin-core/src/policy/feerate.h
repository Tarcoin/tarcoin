...
const std::string CURRENCY_UNIT = "TAR"; // One formatted unit
const std::string CURRENCY_ATOM = "tar"; // One indivisible minimum value unit
...
/**
 * Fee rate in tar per virtualbyte: CAmount / vB
 * the feerate is represented internally as FeeFrac
 */
class CFeeRate
{
private:
    /** Fee rate in tar/vB (satoshis per N virtualbytes) */
    FeePerVSize m_feerate;

public:
    /** Fee rate of 0 tar per 0 vB */
    CFeeRate() = default;
    ...
    /**
     * Construct a fee rate from a fee in tar and a vsize in vB.
     *
    ...
    /**
     * Return the fee in tar for the given vsize in vbytes.
     * If the calculated fee would have fractional tar, then the
     * returned fee will always be rounded up to the nearest tar.
     */
    ...
    /**
     * Return the fee in tar for a vsize of 1000 vbytes
     */
